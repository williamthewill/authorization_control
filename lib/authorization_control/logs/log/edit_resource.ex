defmodule AuthorizationControl.Logs.Log.EditResource do
  ...
  defp build_message(change_agent, %Accounts.Admin{} = resource),
    do:
      ~s(O administrador "#{change_agent.email}" editou o administrador com o email #{
        resource.email
      })

  defp build_message(change_agent, %Access.Operations.Operation{} = resource),
    do: ~s(O administrador "#{change_agent.email}" editou a operação de nome #{resource.name})
